...
type CartItem = {
    id: string;
    name: string;
    base_price: number; // Harga sebelum pajak eksklusif
    final_price: number; // Harga setelah pajak inklusif (sama dengan base_price jika tdk ada pajak inklusif)
    quantity: number;
    taxes: Tax[] | null;
};
...
export function POSClient({ outlets, categories, userName }: { 
        ...
        cart.forEach(item => {
            ...
            if (item.taxes) {
                item.taxes.forEach(tax => {
                    if (!tax.is_inclusive) { // Pajak Eksklusif
                        totalTax += itemSubtotal * (tax.rate / 100);
                    } else { // Pajak Inklusif (pajak sudah termasuk di harga)
                        const priceBeforeTax = item.base_price / (1 + tax.rate / 100);
                        totalTax += (item.base_price - priceBeforeTax) * item.quantity;
                    }
                });
            }
        });
    ...
    const calculatePriceDetails = (product: Product) => {
        let base_price = product.selling_price;
        let final_price = product.selling_price;

        // Jika ada pajak inklusif, final_price tetap, base_price dihitung ulang
        // Note: Logika ini menyederhanakan; jika ada >1 pajak inklusif, perlu penanganan khusus.
        // Untuk saat ini, kita anggap harga jual di DB sudah final.
        // Kalkulasi pajak akan dihitung dari base_price ini saat di keranjang.
    ...
    const handleAddToCart = useCallback((product: Product) => {
        setCart(prevCart => {
            ...
            const { base_price, final_price } = calculatePriceDetails(product);
            return [...prevCart, { 
                id: product.id, 
                name: product.name, 
                base_price,
    ...
    const handleCheckout = async () => {
        ...
        const cartData = cart.map(item => ({ 
            ...
            unit_price: item.base_price // Kirim harga dasar ke backend
        }));
                        ...
                        cart.map(item => (
                            <div key={item.id} className="flex items-center justify-between py-2">
                                <div>
                                    ...
                                    <p className="text-sm text-gray-500">{new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(item.final_price)}</p>
                                </div>

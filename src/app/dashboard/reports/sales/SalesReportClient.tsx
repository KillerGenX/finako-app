...
import { AreaChart, BarChart3, Clock, Users, CreditCard, ShoppingBag, Loader2, Download, Eye, X, PieChart as PieIcon, Crown, UserCheck } from 'lucide-react';
                        ...
                        <tbody>
                            {transaction.items.map((item: TransactionItemDetails, index: number) => (
                                <tr key={index} className="border-b last:border-b-0">
                                    ...
                                    <td className="p-2 text-right">{formatCurrency(item.line_total)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
...
const ReportTable = ({ headers, data, renderRow }: { headers: string[], data: any[], renderRow: (item: any, index: number) => React.ReactNode}) => (
    <div className="overflow-x-auto">
        <table className="w-full text-sm">
            <thead>
                <tr className="border-b">
                    {headers.map((h, i) => <th key={i} className={`p-3 text-left font-medium ${h.startsWith('Jumlah') || h.startsWith('Total') || h.startsWith('Rata-rata') ? 'text-right' : ''}`}>{h}</th>)}
                </tr>
            </thead>
            <tbody>
                {data.map(renderRow)}
...
export function SalesReportClient({ 
    ...
    const PIE_COLORS = ['#0d9488', '#0ea5e9', '#f97316', '#8b5cf6', '#ef4444'];
            ...
            <div className="p-4 bg-white dark:bg-gray-800/50 rounded-lg border mb-6 flex flex-wrap items-end gap-4">
                 <div className="flex items-end gap-2">
                    <div>
                        ...
                        <input id="from" type="date" name="from" value={format(date.from, 'yyyy-MM-dd')} onChange={(e) => setDate(d => ({...d, from: new Date(e.target.value)}))} className="p-2 border rounded w-full mt-1 bg-white dark:bg-gray-800" />
                    </div>
                    <div>
                         <label htmlFor="to" className="text-sm font-medium text-gray-600">Sampai Tanggal</label>
                        <input id="to" type="date" name="to" value={format(date.to, 'yyyy-MM-dd')} onChange={(e) => setDate(d => ({...d, to: new Date(e.target.value)}))} className="p-2 border rounded w-full mt-1 bg-white dark:bg-gray-800" />
                    </div>
                </div>
                <div>
                     <label htmlFor="outlet" className="text-sm font-medium text-gray-600">Outlet</label>
                     <select id="outlet" value={selectedOutlet} onChange={(e) => setSelectedOutlet(e.target.value)} className="p-2 border rounded w-full mt-1 min-w-[150px] bg-white dark:bg-gray-800">
                         <option value="all">Semua Outlet</option>
                         {outlets.map(o => <option key={o.id} value={o.id}>{o.name}</option>)}
                     </select>
                </div>
                <button onClick={handleApplyFilter} disabled={isFiltering || isExporting} className="px-4 py-2 bg-teal-600 text-white rounded disabled:opacity-50 h-10">
                    {isFiltering ? <Loader2 className="animate-spin" /> : "Terapkan"}
                </button>
                <button onClick={handleExport} disabled={isFiltering || isExporting} className="px-4 py-2 bg-green-600 text-white rounded flex items-center gap-2 disabled:opacity-50 h-10">
                    {isExporting ? <Loader2 className="animate-spin" /> : <><Download size={16} /> Export Excel</>}
                </button>
            </div>

            {isFiltering ? (
                <div className="text-center p-12"><Loader2 className="mx-auto h-12 w-12 animate-spin text-teal-600" /><p className="mt-4">Memuat laporan...</p></div>
            ) : !initialData ? (
                <div className="text-center p-12"><p>Gagal memuat data laporan atau tidak ada data pada rentang tanggal ini.</p></div>
            ) : (
                <>
                        ...
                        {activeTab === 'products' && (
                           <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                               <div className="p-4 bg-white dark:bg-gray-800/50 rounded-lg border">
                                    ...
                                    <ReportTable headers={['Produk', 'Jumlah Terjual', 'Total Laba']} data={initialData.top_products}
                                        renderRow={(item, index) => (
                                            <tr key={index} className="border-b last:border-b-0">
                                                <td className="p-3"><p className="font-semibold">{item.product_name}</p><p className="text-xs text-gray-500">{item.template_name}</p></td>
                                                <td className="p-3 text-right">{item.total_quantity_sold}</td>
                        ...
                        {activeTab === 'staff' && (
                             <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                 <div className="p-4 bg-white dark:bg-gray-800/50 rounded-lg border">
                                     ...
                                     <ReportTable headers={['Nama Kasir', 'Jumlah Transaksi', 'Total Penjualan']} data={initialData.cashier_performance}
                                        renderRow={(item, index) => (
                                            <tr key={index} className="border-b last:border-b-0">
                                                <td className="p-3 font-semibold">{item.cashier_name}</td>
                                                <td className="p-3 text-right">{item.transaction_count}</td>
                                                <td className="p-3 text-right">{formatCurrency(item.net_revenue)}</td>
                                            </tr>
                                        )}
                                     />
                                 </div>
                                  <div className="p-4 bg-white dark:bg-gray-800/50 rounded-lg border">
                                     ...
                                     <ReportTable headers={['Nama Pelanggan', 'Jumlah Transaksi', 'Total Belanja']} data={initialData.top_customers}
                                        renderRow={(item, index) => (
                                            <tr key={index} className="border-b last:border-b-0">
                                                <td className="p-3 font-semibold">{item.customer_name}</td>
                                                <td className="p-3 text-right">{item.transaction_count}</td>
                                                <td className="p-3 text-right">{formatCurrency(item.total_spent)}</td>
                                            </tr>
                                        )}
                                     />
                                 </div>
                                ...
                                <ReportTable headers={['No. Transaksi', 'Tanggal', 'Kasir', 'Pelanggan', 'Total', 'Aksi']} data={initialData.transaction_history}
                                    renderRow={(tx: TransactionHistoryEntry, index) => (
                                        <tr key={tx.id} className="border-b last:border-b-0">
                                            ...
                                            <td className="p-3">{tx.customer_name || 'Umum'}</td>
                                            <td className="p-3 text-right font-semibold">{formatCurrency(tx.grand_total)}</td>
                                            <td className="p-3 text-center"><button onClick={() => setSelectedTransaction(tx)} className="p-1 text-teal-600 hover:text-teal-800"><Eye size={16} /></button></td>
                                        </tr>
                                    )}
                                />
                           </div>

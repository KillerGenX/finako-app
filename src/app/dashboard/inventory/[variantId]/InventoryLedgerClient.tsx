...
function InitialStockModal({ outlets, variantId, onClose, onSave }: {
    ...
    const handleSave = () => {
        ...
        stocks.forEach((qty, id) => {
            if (qty > 0) stocksToSave.push({ outlet_id: id, quantity: qty });
        });
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex justify-center items-center p-4">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-lg">
                ...
                <div className="p-6 border-y max-h-80 overflow-y-auto">
                    {outlets.map(outlet => (
                        <div key={outlet.id} className="flex items-center justify-between mb-2">
                            <label>{outlet.name}</label>
                            <input type="number" min="0" className="w-24 p-1 border rounded text-center" onChange={e => setStocks(prev => new Map(prev).set(outlet.id, parseInt(e.target.value, 10)))} />
                        </div>
                    ))}
                </div>
...
// Komponen Buku Besar
function StockLedgerTable({ ledger }: { ledger: LedgerDetails['ledger'] }) {
        ...
        <div className="overflow-x-auto max-h-[60vh]">
            <table className="min-w-full text-sm">
                <thead className="text-left text-gray-500"><tr>
                    <th className="p-2">Tanggal</th><th className="p-2">Tipe</th><th className="p-2">Outlet</th><th className="p-2">Referensi</th><th className="p-2 text-right">Perubahan</th>
                </tr></thead>
                <tbody>
                    {ledger.map((move, i) => (
                        <tr key={i} className="border-t">
                            <td className="p-2">{formatDate(move.created_at)}</td>
                            <td className="p-2">{move.movement_type}</td>
                            <td className="p-2">{move.outlet_name}</td>
                            <td className="p-2 text-teal-600 hover:underline font-mono">{move.reference_number || '-'}</td>
                            <td className={`p-2 text-right font-medium ${move.quantity_change > 0 ? 'text-green-600' : 'text-red-600'}`}>
...
export function InventoryLedgerClient({ productVariant, outlets, initialDetails, hasInitialStock }: {
    ...
    return (
        <>
            ...
            {/* Header */}
            <div className="mb-6"><Link href="/dashboard/products" className="inline-flex items-center text-sm font-medium text-gray-600 hover:text-gray-900"><ArrowLeft className="h-4 w-4 mr-2" />Kembali ke Daftar Produk</Link><h1 className="text-3xl font-bold tracking-tight mt-2">Pusat Komando Stok</h1><p className="mt-1 text-md text-gray-600">{productVariant.name} <span className="text-xs bg-gray-200 p-1 rounded">{productVariant.sku || 'No SKU'}</span></p></div>

            {/* Konten Utama */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-1 space-y-6">
                    {/* Kartu Informasi & Aksi */}
                    <div className="p-6 bg-white dark:bg-gray-900/50 rounded-lg border"><h2 className="font-semibold mb-1">Total Stok Saat Ini</h2><p className="text-4xl font-bold">{details.summary.total_stock} <span className="text-lg font-normal">unit</span></p></div>
                    ...
                    {!hasInitialStock && (
                        <div className="p-6 bg-yellow-50 border-yellow-200 rounded-lg border text-center">
                            <h2 className="font-semibold mb-2">Produk Baru!</h2>
                            <p className="text-sm text-yellow-800 mb-4">Atur saldo stok awal untuk produk ini untuk memulai pelacakan inventaris.</p>
                            <button onClick={() => setShowInitialStockModal(true)} className="w-full inline-flex items-center justify-center px-4 py-2 bg-yellow-400 text-yellow-900 rounded-lg hover:bg-yellow-500 font-semibold"><CheckCircle className="h-5 w-5 mr-2" /> Atur Stok Awal</button>
                        </div>
                    )}
                ...
                <div className="lg:col-span-2 p-6 bg-white dark:bg-gray-900/50 rounded-lg border">
                    ...
                    <div style={{width: '100%', height: 200}}>
                        <ResponsiveContainer><LineChart data={details.chart_data}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="date" /><YAxis /><Tooltip /><Line type="monotone" dataKey="stock" stroke="#0d9488" strokeWidth={2} /></LineChart></ResponsiveContainer>
                    </div>

                    {/* Buku Besar Stok */}
                    <h2 className="font-semibold my-4 flex items-center gap-2"><History size={18} /> Buku Besar Stok</h2>
                    <StockLedgerTable ledger={details.ledger} />
                </div>
            </div>
        </>
    );

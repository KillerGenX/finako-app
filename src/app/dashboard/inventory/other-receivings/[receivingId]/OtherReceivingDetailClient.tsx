...
import { OtherReceivingDetails } from './actions';

// --- Komponen Tampilan Cetak ---
function PrintableView({ details }: { details: OtherReceivingDetails }) {
    ...
    return (
        <div className="bg-white text-black p-8 font-sans">
            <div className="flex justify-between items-start pb-4 border-b">
                ...
                <div className="text-right"><h2 className="text-2xl font-bold">BUKTI PENERIMAAN BARANG (LAINNYA)</h2><p className="text-sm font-mono">{details.receiving_number}</p></div>
            </div>
            <div className="my-4"><p className="text-sm"><span className="font-semibold">Tanggal Diterima:</span> {formatDate(details.created_at)}</p></div>
            <table className="w-full text-sm my-8">
                <thead className="bg-gray-100"><tr>
                    ...
                    <th className="p-2 text-left font-semibold border">Catatan</th>
                </tr></thead>
                <tbody>
...
export function OtherReceivingDetailClient({ details }: { details: OtherReceivingDetails }) {
    if (!details) return null;
            ...
            <div className="no-print">
                <div className="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <h1 className="text-2xl font-bold">Detail Penerimaan #{details.receiving_number}</h1>
                    </div>
                ...
                <div className="border rounded-lg w-full bg-white dark:bg-gray-800/50">
                    <table className="w-full text-sm">
                        <thead><tr className="border-b"><th className="p-4 text-left">Produk</th><th className="p-4 text-left">SKU</th><th className="p-4 text-left">Catatan Item</th><th className="p-4 text-right">Jumlah</th></tr></thead>
                        <tbody>
                            {details.items.map(item => (<tr key={item.id} className="border-b"><td className="p-4 font-semibold">{item.name}</td><td className="p-4 text-gray-500">{item.sku}</td><td className="p-4">{item.notes}</td><td className="p-4 text-right font-mono font-semibold">{item.quantity}</td></tr>))}
                        </tbody>
                    </table>
                </div>
            </div>

...
const StockAdjustmentSchema = z.object({
    ...
    movement_type: z.enum(['purchase_received', 'adjustment', 'initial_stock']),
...
const StockTransferSchema = z.object({
    ...
    path: ["outlet_to_id"], // Error message will be shown on the 'to' field
});
...
export async function adjustStock(prevState: StockFormState, formData: FormData): Promise<StockFormState> {
    try {
        ...
        const { product_variant_id, outlet_id, quantity_change, movement_type } = validatedFields.data;
    ...
    revalidatePath(`/dashboard/products/${formData.get('product_variant_id')}/inventory`);
...
export async function transferStock(prevState: StockFormState, formData: FormData): Promise<StockFormState> {
     try {
        ...
        if (error) {
            // Catch specific errors from the DB function to give user-friendly feedback

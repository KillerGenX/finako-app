...
const FormInput = ({ id, label, type, required, error, step, defaultValue, placeholder }: { id: string, label: string, type: string, required?: boolean, error?: string[], step?: string, defaultValue: string | number | null, placeholder?: string }) => (
    <div>
...
export function SingleVariantEditor({ variant, productId }: { variant: Variant, productId: string }) {
            ...
            <form action={formAction}>
                ...
                <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <FormInput id="cost_price" label="Harga Pokok" type="number" defaultValue={variant.cost_price} step="0.01" error={state.errors?.cost_price} placeholder="Opsional"/>
                    </div>
                    ...
                    <div className="pt-2">
                        <label className="flex items-center gap-2 font-normal cursor-pointer">
                            <input type="checkbox" name="track_stock" id="track_stock" defaultChecked={variant.track_stock} className="form-checkbox rounded text-teal-600" />
